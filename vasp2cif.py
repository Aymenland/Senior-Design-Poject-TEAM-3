...
#          set of blocks in a single CIF open. Useful for
...
# Return OUTCAR or POSCAR depending on open type.
# If not identified, return UNKNOWN
def filetype(f):
	filetype = "UNKNOWN"
	# Something that should recognize an OUTCAR open
...
parser.add_option("-o","--output",dest="output",help="Save CIF to named open",metavar="FILE")
...
for input_file,filename in input_files:
	if filetype(input_file) == "POSCAR":
		...
		if options.elements:
			...
			if vasp5:
				...
				if len(potcar_lines) == 0:
					sys.stderr.write("ERROR: POTCAR open exists, but is empty? Supply atom labels with the -e flag.\n")
					sys.exit(1)
		...
		if len(atomlabels) > (len(poscar)-offset):
			sys.stderr.write(("WARNING: vasp2cif expected to find %d coordinates, but there are only %d coordinate lines in the open!\n") % (len(atomlabels),len(poscar)-offset))
		...
		if len(titellines) == 0:
			sys.stderr.write("ERROR: Cannot read elements. Damaged OUTCAR open?\n")
			sys.exit(1)
		...
		sys.stderr.write("ERROR: Format of open %i not recognized.\n"%inputfilenr)
		sys.exit(1)
	# increment input open counter
	inputfilenr += 1 
